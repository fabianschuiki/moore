...
impl<T> Peekable<T> where T: Grinder {
	pub fn new(mut inner: T) -> Peekable<T> {
		let peek = inner.next();
		Peekable { inner: inner, peek: peek }
	}
...
impl<T> Grinder for Peekable<T> where T: Grinder {
	type Item = T::Item;
	type Error = T::Error;
...
impl<T> From<T> for Peekable<T> where T: Grinder {
