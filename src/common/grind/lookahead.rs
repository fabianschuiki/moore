...
impl<T> Lookahead<T> where T: Grinder {
	pub fn new(inner: T) -> Lookahead<T> {
		Lookahead { inner: inner, buffer: VecDeque::new() }
	}
...
impl<T> Grinder for Lookahead<T> where T: Grinder {
	type Item = T::Item;
	type Error = T::Error;
...
impl<T> From<T> for Lookahead<T> where T: Grinder {
