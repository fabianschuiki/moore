    ...
    C: Context<'gcx> + 'b,
{
    ...
    fn emit_module_block(
        ...
        for &inst_id in &hir.insts {
            let inst_hir = match self.hir_of(inst_id)? {
                HirNode::Inst(x) => x,
                _ => unreachable!(),
            };
            let target_hir = match self.hir_of(inst_hir.target)? {
                HirNode::InstTarget(x) => x,
                _ => unreachable!(),
            };

            // Resolve the name of the instantiated module.
            let module = match self.gcx().find_module(target_hir.name.value) {
                Some(id) => id,
                None => {
                    self.emit(
                        DiagBuilder2::error(format!(
            ...
            trace!("pos_ports = {:#?}", inst_hir.pos_ports);
            trace!("named_ports = {:#?}", inst_hir.named_ports);
            trace!("has_wildcard_port = {:#?}", inst_hir.has_wildcard_port);
            ...
            trace!("External Port Mapping: {:?}", port_mapping);

            let module_hir = match self.hir_of(module)? {
                HirNode::Module(x) => x,
                _ => unreachable!(),
            };
